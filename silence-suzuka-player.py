...
class MarqueeLabel(QLabel):
    ...
    def setText(self, text):
        ...
        self._offset = 0
        self._text = text or ""
        QTimer.singleShot(0, self._update_animation_state)
    ...
    def _update_animation_state(self):
        ...
        should_be_active = self._text_width > self.width()
    ...
    def paintEvent(self, event):
        ...
        gap = 40
        x = self._offset
        y = int((self.height() - painter.fontMetrics().height()) / 2) + painter.fontMetrics().ascent()
        while x < self.width():
    ...
    def _update_scroll(self):
        ...
        if self._offset <= -(self._text_width + gap):
            self._offset = 0
        self.update()
...
class MediaPlayer(QMainWindow):
    ...
    def _build_ui(self):
        ...
        # Track Title Label
        self.track_label = MarqueeLabel("No track playing")
    ...
    def play_current(self):
            ...
            # Skip items previously completed
            ...
            _key = self._canonical_url_key(url) if url else None
            _resume_ms = int(self.playback_positions.get(_key, self.playback_positions.get(url, 0))) if url else 0
            ...
            try:
                if _resume_sec > 0:
                    self.mpv.command('loadfile', url, 'replace', f'start={_resume_sec}')
                else:
            ...
            if _resume_ms > 0:
                self._restore_saved_position_attempt(url, _resume_ms, 1)
